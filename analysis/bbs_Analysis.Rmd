...
load("~/Documents/functional_diversity/data/trait.rda")
load("~/Documents/functional_diversity/data/bbs.rda")
...
region_name <- "SOUTHERN.ROCKIES.COLORADO.PLATEAU"
region_stats <- read_tsv(paste0("data/stat/", region_name, ".tsv.bz2"))

region_data <- dplyr::filter(bbs_site_FD, region == "APPALACHIAN MOUNTAINS") %>%
    select(-sing.sp, -starts_with("CWM")) %>%
    gather(metric_names, key = "metric", value = "value") %>%

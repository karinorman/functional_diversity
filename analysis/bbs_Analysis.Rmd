...
load("~/functional_diversity/data/trait.rda")
load("~/functional_diversity/data/bbs.rda")
...
simulate_null <- function(region_data, n = 1000){
  ...
  null_samples <- function(richness){
    fds <- map_dfr(1:n, possibly(get_sample_fd, data.frame()), richness = richness)
  ...
  #test_sim <- future_map_dfr(richness_levels, null_samples, .progress = TRUE)
  test_sim <- mclapply(richness_levels, null_samples, mc.cores = 30)
}
...
get_regional_sig_test <- function(region_name, ...){
  ...
  species_pool <- bbs %>% 
    filter(year > min_year & site_id %in% unique(region_FD$site_id)) %>%
    #left_join(., select(region_FD, site_id, geometry), by = "site_id") %>% #get the geometry column for mapping
    select(scientific) %>%
    unique() #get column of unique scientific names
  
  plan(multicore)
  system.time({null_stats <- simulate_null(region_FD, ...)})
  
  unique(region_FD$site_id)
...
get_metric_sig <- function(){
  if(!dir.exists("~/functional_diversity/data/stat")){
    dir.create("~/functional_diversity/data/sim")
...
n_rockies <- get_regional_sig_test(region_name = "APPALACHIAN MOUNTAINS", n = 10)

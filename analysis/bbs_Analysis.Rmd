...
get_regional_sig_test <- function(region_name, ...){
  ...
  write_tsv(null_stats, paste0("~/functional_diversity/data-raw/stat/", region_name, ".tsv.bz2"))
  ...
  write_tsv(FD_sig, paste0("~/functional_diversity/data-raw/stat_test/", region_name, ".tsv.bz2"))
  
  return(TRUE)
}

dir.create("~/functional_diversity/data-raw/sim")
dir.create("~/functional_diversity/data-raw/stat_test")
n_rockies <- get_regional_sig_test("NORTHERN ROCKIES", n = 10)

out <- map(unique(bbs_site_FD$region)[1:10], get_regional_sig_test, n=10)
stat_test <- purrr::map_dfr(fs::dir_ls(path = "~/functional_diversity/data-raw/stat_test/", glob="*.tsv.bz2"), readr::read_tsv, .id = "region")

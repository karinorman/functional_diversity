...
simulate_null <- function(region_data, n = 1000){
  ...
  null_samples <- function(richness){
    test <- map(1:n, possibly(get_sample_fd, data.frame()), richness = richness) %>% 
      bind_rows() %>%
      select(-nbsp, -sing.sp) %>%
  ...
  test_sim <- map(richness_levels, null_samples) %>% 
    bind_rows()
...
#check if each northern rockies site falls within 
...
check_site_sig <- function(site_data){
  map(metric_names, check_metric_sig) %>%
    bind_cols() %>%

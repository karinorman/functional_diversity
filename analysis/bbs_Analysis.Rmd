...
get_sample_fd <- function(x, ...){
  ...
  return(head(sample_FD, -1))
}

simulate_null <- function(){
  test_sim <- map(1:1000, function(x){ 
  map(10:110, possibly(get_sample_fd, data.frame())) %>% bind_rows()
  }) %>% bind_rows()
}
...
#system.time({test_sim <- plyr::ldply(10:110, get_sample_fd()$species)}) #would work if dbFD didn't error out

#Calculate FD for each sampled community of the given richness, skip richness level if it errors(probably due to memory)
system.time({ FDdf <- data.frame()
for(i in 10:110){

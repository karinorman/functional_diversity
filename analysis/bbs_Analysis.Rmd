...
get_region_samples <- function(region_name){
  ...
  region_samples %>%
    mutate(value = 1) %>%
    arrange(samp_species) %>% #get species in alphabetical order
    spread(samp_species, value) %>%
    select(-richness)
    
}

first_sample <- map_dfr(unique(bbs_site_FD$region), get_region_samples) %>%
    select(sort(current_vars())) #sort columns to be in alphabetical order, required for dbFDI()
sample_trait_mat <- get_trait_matrix(colnames(first_sample))
sample_fd <- dbFD(x = sample_trait_mat, a = first_sample, w.abun = FALSE)
